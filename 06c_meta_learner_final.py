...
!pip install transformers peft accelerate torchao scikit-learn xgboost --upgrade -q
...
print("Loading DeBERTa V2 Fixed (with pooler)...")
...
# Load checkpoint
epoch3_path = f"{DIRS['checkpoints2_v2_fixed']}/epoch_3"
...
# Load OOF predictions (fold 4 = full 25k)
roberta_oof = np.load(f"{DIRS['oof_v2']}/roberta_v2_fold4.npy")
deberta_oof = np.load(f"{DIRS['oof_v2_fixed']}/deberta_v2_fixed_fold4.npy")
...
# Meta feature matrix for testing
...
results.append(res)
print(f"✓ Logistic Regression — RoBERTa w: {lr.coef_[0][0]:.4f}, DeBERTa w: {lr.coef_[0][1]:.4f}")
...
print(f"\n{'='*75}")
print("META-LEARNER FINAL RESULTS (evaluated on 25k test set)")
print(f"{'='*75}")
...
plt.title("ROC Curves — Final Meta-Learner Comparison")
plt.legend(loc="lower right")
plt.tight_layout()
plt.savefig(f"{DIRS['results']}/roc_curves_final.png", dpi=150)
plt.show()

# Save results
save_results({"meta_learner_final": results},
             "meta_learner_final_results.json")
print("✓ Results saved.")
